...
space = IMAGE_SIZE / GRID_SIZE   # = 15

def create_grid_drones(rows, cols, spacing=space):
    drones = []
    for i in range(rows):
        for j in range(cols):
...
def move_toward_target(drone, step_size=1):
...
def visualize_3d(ax, drones):
    ...
    if len(active) > 0:
        ax.scatter(active[:, 0], active[:, 1], active[:, 2],
                   c='blue', s=5, label='Flying')

    if len(finished) > 0:
        ax.scatter(finished[:, 0], finished[:, 1], finished[:, 2],
                   c='green', s=5, label='Arrived')
